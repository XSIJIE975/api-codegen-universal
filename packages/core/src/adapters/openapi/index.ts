...
export class OpenAPIAdapter implements IAdapter {
  ...
  async parse(
    ...
    if (options?.commonPrefix) {
      this.pathClassifier = new PathClassifier({
        ...
        maxDepth: options.maxClassificationDepth || 2,
      })
    }
    
    // 设置命名风格
    this.namingStyle = options?.parameterNamingStyle || 'PascalCase'
    ...
    return { schemas, interfaces, apis, metadata }
  }
  ...
  private extractAPIs(
    ...
    for (const pathMember of pathsNode.members) {
      if (ts.isPropertySignature(pathMember) && pathMember.name && pathMember.type) {
        ...
        if (path && ts.isTypeLiteralNode(pathMember.type)) {
          ...
          for (const methodMember of pathMember.type.members) {
            if (ts.isPropertySignature(methodMember) && methodMember.name && methodMember.type) {
              const method = (methodMember.name as ts.Identifier).text.toUpperCase()
              
              // 提取 operationId 引用
              const operationId = this.extractOperationIdReference(methodMember.type)
          ...
          for (const schemaMember of member.type.members) {
            if (ts.isPropertySignature(schemaMember) && schemaMember.name && schemaMember.type) {
              ...
              if (schemaName && ts.isTypeLiteralNode(schemaMember.type)) {
                // 解析具体的 schema 结构
                schemas[schemaName] = this.typeNodeToSchema(schemaName, schemaMember.type)
              }
            }
          }
        }
  ...
  private buildApiDefinition(
    ...
    for (const member of operationNode.members) {
      if (ts.isPropertySignature(member) && member.name) {
        ...
        if (propName === 'parameters' && member.type && ts.isTypeLiteralNode(member.type)) {
          // 提取 parameters (query/path/header/cookie)
          api.parameters = this.extractParameters(operationId, member.type, schemas, interfaces)
  ...
  private extractParameters(
    ...
    for (const member of parametersNode.members) {
      if (ts.isPropertySignature(member) && member.name && member.type) {
        const location = (member.name as ts.Identifier).text as 'query' | 'path' | 'header' | 'cookie'
        ...
        if (ts.isTypeLiteralNode(member.type)) {
          // 生成接口名称（使用配置的命名风格）
          const interfaceName = this.generateParameterInterfaceName(operationId, location)
          ...
          // 生成接口代码
          const interfaceCode = this.generateInterfaceString(interfaceName, member.type, false)
   ...
   * 生成参数接口名称
   */
  private generateParameterInterfaceName(operationId: string, location: string): string {
    // 转换 location 首字母大写
    const locationCapitalized = location.charAt(0).toUpperCase() + location.slice(1)
    
    // 根据命名风格转换
    return this.convertToNamingStyle(`${operationId}_${locationCapitalized}_Params`)
  }
  ...
  private convertToNamingStyle(name: string): string {
    switch (this.namingStyle) {
      ...
      case 'camelCase':
        ...
        return firstPart.charAt(0).toLowerCase() + firstPart.slice(1) + 
      ...
      case 'snake_case':
        ...
        return name.toLowerCase().replace(/[A-Z]/g, (letter, index) => 
   ...
   * 提取 requestBody 定义
   */
  private extractRequestBody(typeNode: ts.TypeNode): ApiDefinition['requestBody'] {
    ...
    for (const member of typeNode.members) {
      if (ts.isPropertySignature(member) && member.name) {
        ...
        if (propName === 'content' && member.type && ts.isTypeLiteralNode(member.type)) {
          const content: Record<string, any> = {}
          
          // 遍历 content-type
          for (const contentMember of member.type.members) {
            if (ts.isPropertySignature(contentMember) && contentMember.name) {
              const contentType = this.extractStringFromNode(contentMember.name)
              
              if (contentType && contentMember.type) {
                // 提取 schema 引用
                const schemaRef = this.extractSchemaReference(contentMember.type)
        ...
        if (statusCode && ts.isTypeLiteralNode(member.type)) {
          ...
          for (const respMember of member.type.members) {
            if (ts.isPropertySignature(respMember) && respMember.name) {
              ...
              if (propName === 'content' && respMember.type && ts.isTypeLiteralNode(respMember.type)) {
                // 查找 application/json
                for (const contentMember of respMember.type.members) {
                  if (ts.isPropertySignature(contentMember) && contentMember.name) {
                    const contentType = this.extractStringFromNode(contentMember.name)
                    
                    if (contentType === 'application/json' && contentMember.type) {
                      ...
                      const typeText = printer.printNode(ts.EmitHint.Unspecified, contentMember.type, sourceFile)
                      const genericResult = this.genericDetector.detect(typeText)
                      
                      let schemaRef: SchemaReference
                      if (genericResult.isGeneric && genericResult.baseType && genericResult.genericParam) {
                        // 泛型类型 - 记录基类
                        ...
                        // 普通引用
                        const schemaName = this.extractSchemaReference(contentMember.type)
                        schemaRef = {
                          type: 'ref',
   ...
   */
  private typeNodeToSchema(name: string, typeNode: ts.TypeNode): SchemaDefinition {
  ...
  private generateInterfaceString(
    ...
    lines.push(`interface ${name}${genericPart} {`)
    ...
    for (const member of typeNode.members) {
      if (ts.isPropertySignature(member) && member.name && member.type) {
        ...
        // 打印整个成员节点(包括注释)
        const memberText = printer.printNode(ts.EmitHint.Unspecified, member, sourceFile)
  ...
  private typeNodeToString(typeNode: ts.TypeNode): string {
    ...
    // 打印类型节点
    let typeStr = printer.printNode(ts.EmitHint.Unspecified, typeNode, sourceFile)
